...
const StatsOverview: React.FC = () => {
  ...
  const bestStreak =
    s.habits?.length > 0 ? Math.max(...s.habits.map((h) => h.streak)) : 0;

  // 2. Calculate Total Spark Points (Total completions ever)
  const totalSparkPoints = s.habits.reduce(
    (acc, h) => acc + h.history.length,
    0,
  );

  // 3. Calculate Today's Progress
  const completedToday = s.habits.filter((h) =>
    h.history.includes(today),
  ).length;
  const progressPercent =
    s.habits?.length > 0
      ? Math.round((completedToday / s.habits.length) * 100)
      : 0;
