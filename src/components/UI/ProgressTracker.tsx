...
const ProgressTrack: React.FC<ProgressTrackProps> = ({ radius, stroke, progress }) => {
  ...
  return (
    <svg width={radius * 2} height={radius * 2} style={{ display: "block", margin: "0 auto" }}>
      ...
      <circle
        ...
        cy={radius}
      />
      <text x="50%" y="50%" textAnchor="middle" dy=".3em" fontSize="1.1em" fill="var(--color-text-primary)">
        {progress}%
      </text>
    </svg>
  );
