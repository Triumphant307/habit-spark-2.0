import type { WCPaths, PathValue } from "./types";
...
export function assignAny<T extends object>(
  ...
  keys.forEach((key, i) => {
    ...
    if (match) {
      ...
      else (currObj[k][Number(idx)] ||= {}), (currObj = currObj[k][Number(idx)]);
    } else {
...
export function getTrailPaths<T>(path: WCPaths<T>, reverse = true): WCPaths<T>[] {
...
export function getTrailRecord<T extends object>(
  ...
  for (let i = 0; i < parts.length; i++) {
    acc += (i === 0 ? "" : ".") + parts[i];
    record.push([acc as WCPaths<T>, currObj, (currObj = Reflect.get(currObj, parts[i]))]);
  }
  return record;
}
